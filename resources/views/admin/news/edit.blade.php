...
@section('content')
    ...
    <form class="form-horizontal" method="POST" action="{{ route('news.update',  $news->slug) }}" enctype="multipart/form-data">
        {{ csrf_field() }}
        {{ method_field('PATCH') }}
    ...
    <div class="modal fade" id="modal-deletefile" tabIndex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form method="POST" action="{{ route('news.deletefile', $news->slug) }}">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                        <button type="button" class="btn btn-default"
                            data-dismiss="modal">Close</button>
